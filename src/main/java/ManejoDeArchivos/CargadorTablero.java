...
public class CargadorTablero {
    ...
    public void formarTablero() {
        for (int k = 0; k < lineasArchivo.size(); k++) {
            generarEspacioTablero(lineasArchivo.get(k), k);
        }
    }

    public void generarEspacioTablero(String condicion, int k) {
        ...
        try {
            switch (primeraDivision[0]) {
                case TAMANO:
                    this.tablero = new Tablero(Integer.valueOf(segundaDivision[1]), Integer.valueOf(segundaDivision[0]));
                    break;
                case AVANZAR:
